...
const newUser = computed(() => userStore.editedUser)
...
useAsyncData('users', async () => {
    ...
    await userStore.getEditedUser(id);
}, { server: true });
...
const saveChanges = () => {
  if (user.value.name !== newUser.value.name) {
    userStore.editUser('name', newUser.value.name, newUser.value._id);
  }
  if (user.value.post !== newUser.value.post) {
    userStore.editUser('post', newUser.value.post, newUser.value._id);
  }
  if (photos.value.length > 0) {
    userStore.editUser('avatar',photos.value[0], newUser.value._id);
  }
    ...
    <div class=" w-full h-full mt-20 px-4 md:px-12 lg:px-24 xl:px-36 2xl:px-48">
      <div class="relative  border-2 border-[#  696969] rounded-[20px] flex flex-col items-center p-20">
          ...
          <form class="flex flex-col items-start gap-5 max-w-[324px] z-20">
            <input v-model="newUser.name" type="text" placeholder="Имя" class="bg-transparent border-b border-[#979797] focus:outline-none w-full"/>
            <input v-model="newUser.post" type="text" placeholder="Должность" class="bg-transparent border-b border-[#979797] focus:outline-none w-full"/>
            ...
            <input class="pb-10" type="file" @change="handleFileUpload">
